...
def _count_iocs(content: str) -> dict[str, int]:
    ...
    for match in matches:
        # Only count if there is actual OCR text (not just "No OCR text extracted")
...
def _remove_analyst_templates(content: str) -> str:
    ...
    # Remove Executive Summary template (including Workflow Feedback subsection)
    content = re.sub(
    ...
    # Fallback for older format without Workflow Feedback
    content = re.sub(
        r'## Executive Summary \(Analyst Fill-In\)\n+### Key Takeaways\n.*?### Initial Assessment\n.*?(?=\n##)',
        '',
        content,
...
def _add_summary_stats(content: str, counts: dict[str, int]) -> str:
    ...
    if insertion_point == -1:
        return content  # Cannot find insertion point, skip
