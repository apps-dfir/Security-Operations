...
@dataclass
class PreviouslySeenIOC:
    """An IOC that was previously seen in other reports."""
    ioc_value: str
    ioc_type: str
    previous_reports: list  # List of {report, issue_number, title}
...
def _compute_file_hash(file_path: str) -> str:
    """Compute SHA256 hash of a file. Returns empty string if file does not exist."""
    p = Path(file_path)
    if not p.exists():
...
def load_ioc_database(db_path: Path = None) -> dict:
    """Load the IOC database for cross-referencing previous reports."""
    if db_path is None:
        db_path = Path("data/ioc_database.json")
    
    if not db_path.exists():
        return {"ioc_index": {}, "reports": {}}
...
def find_previously_seen_iocs(
    consolidated_iocs: dict,
    ioc_database: dict,
    current_issue_number: Optional[int] = None
) -> list:
    """
    Cross-reference extracted IOCs against the database to find previously seen IOCs.
    
    Args:
        consolidated_iocs: Dict of IOC type -> {ioc_value -> data}
        ioc_database: The loaded IOC database
        current_issue_number: Current issue number to exclude from results
    
    Returns:
        List of PreviouslySeenIOC objects
    """
    previously_seen = []
    ioc_index = ioc_database.get("ioc_index", {})
    
    # Map our IOC type names to database key prefixes
    type_prefix_map = {
        'cves': 'cves',
        'urls': 'urls',
        'domains': 'domains',
    ...
    for ioc_type, iocs_dict in consolidated_iocs.items():
        prefix = type_prefix_map.get(ioc_type, ioc_type)
        
        for ioc_value in iocs_dict.keys():
            # Build the database key (type:value format, lowercase)
            db_key = f"{prefix}:{ioc_value.lower()}"
            
            if db_key in ioc_index:
                # Found in previous reports
                previous_reports = []
                for entry in ioc_index[db_key]:
                    # Skip if it's from the current issue
                    if current_issue_number and entry.get("issue_number") == current_issue_number:
                        continue
                ...
                if previous_reports:
                    previously_seen.append(PreviouslySeenIOC(
                        ioc_value=ioc_value,
                        ioc_type=ioc_type,
...
def build_consolidated_markdown(inputs: ConsolidatedInputs, previously_seen_iocs: list = None) -> str:
    ...
    if previously_seen_iocs:
        lines.append(f"**Previously Seen IOCs:** {len(previously_seen_iocs)}<br>")
    lines.append(f"**Breakdown:** {breakdown}")
    lines.append("")
    
    # Previously Seen IOCs Section (NEW FEATURE)
    if previously_seen_iocs:
        lines.append("## 🔄 Previously Seen IOCs")
        lines.append("")
        lines.append("> The following IOCs were found in previous reports. This may indicate shared infrastructure, ongoing campaigns, or related threat activity.")
        lines.append("")
        lines.append("| IOC | Type | Previous Reports |")
        lines.append("|-----|------|------------------|")
        
        # Sort by type then by IOC value
        sorted_prev = sorted(previously_seen_iocs, key=lambda x: (x.ioc_type, x.ioc_value))
        
        for prev_ioc in sorted_prev:
            # Defang the IOC for display
            display_value = prev_ioc.ioc_value
            if prev_ioc.ioc_type in ('urls', 'domains', 'ipv4'):
                display_value = display_value.replace("http://", "hxxp://").replace("https://", "hxxps://").replace(".", "[.]")
            
            # Format previous report links
            report_links = []
            for report in prev_ioc.previous_reports:
                issue_num = report.get("issue_number")
                title = report.get("title", "Unknown")
                if issue_num:
                    report_links.append(f"[#{issue_num}: {title}](../../issues/{issue_num})")
                else:
            ...
            type_display = prev_ioc.ioc_type.upper()
            lines.append(f"| `{display_value}` | {type_display} | {', '.join(report_links)} |")
    ...
    lines.append("")
    lines.append("### Workflow Feedback (Analyst Fill-In)")
    lines.append("- Extraction quality: (good / acceptable / poor)")
    lines.append("- False positives noted: (none / some / many)")
    lines.append("- Missing IOCs observed: (none / list below)")
    lines.append("- Parsing issues: (none / describe)")
...
def main() -> int:
    ...
    ap.add_argument("--ioc-db-path", default="data/ioc_database.json", help="Path to IOC database for overlap detection")
    args = ap.parse_args()
    ...
    # Load IOC database for overlap detection
    ioc_database = load_ioc_database(Path(args.ioc_db_path))
    if ioc_database.get("ioc_index"):
        print(f"Loaded IOC database with {len(ioc_database.get('ioc_index', {}))} indexed IOCs")
    else:
        print("No existing IOC database found - overlap detection skipped")
    ...
    # Build consolidated IOCs for overlap detection
    consolidated_iocs_for_lookup = {
        'cves': {},
        'urls': {},
        'domains': {},
        'ipv4': {},
    ...
    for src in processed_sources:
        for ioc_type, iocs_list in src.iocs.items():
            if ioc_type in consolidated_iocs_for_lookup:
                for scored_ioc in iocs_list:
                    ioc_value = scored_ioc.value
                    if ioc_value not in consolidated_iocs_for_lookup[ioc_type]:
                        consolidated_iocs_for_lookup[ioc_type][ioc_value] = True
    
    # Find previously seen IOCs
    current_issue = int(args.issue_number) if str(args.issue_number).isdigit() else None
    previously_seen_iocs = find_previously_seen_iocs(consolidated_iocs_for_lookup, ioc_database, current_issue)
    
    if previously_seen_iocs:
        print(f"Found {len(previously_seen_iocs)} IOCs that appear in previous reports")
    ...
    md = build_consolidated_markdown(inputs, previously_seen_iocs)
